...
class VecDB:
    ...
    def retrieve(self, query: Annotated[np.ndarray, (1, DIMENSION)], top_k=5):
        results = []
        query_signature = self._generate_signature(query.flatten())
        
        # Compute Hamming distances between query_signature and all signatures
        signature_distances = []
        for signature in self.signatures:
            hamming_distance = sum(q_bit != s_bit for q_bit, s_bit in zip(query_signature, signature))
            signature_distances.append((hamming_distance, signature))
        
        # Sort signatures by Hamming distance
        signature_distances.sort(key=lambda x: x[0])
        
        checked_signatures = set()
        for distance, signature in signature_distances:
            signature_tuple = tuple(signature)
            if signature_tuple in checked_signatures:
                continue
            checked_signatures.add(signature_tuple)
            ...
            if os.path.exists(signature_file_path):
                with open(signature_file_path, 'r') as f:
                    for line in f:
                        ...
                        score = self._cal_score(query.flatten(), vector)
                        results.append((row_num, score))
                if len(results) >= top_k:
                    break
        # Sort results and return top_k items
        results.sort(key=lambda x: x[1], reverse=True)
        final_results = [row_num for row_num, score in results[:top_k]]
